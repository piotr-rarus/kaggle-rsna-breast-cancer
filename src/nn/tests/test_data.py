...
def test_dicom_test_dataset(
    ...
    assert torch.allclose(image.mean(), torch.tensor(121.4466), atol=1e-3)
    assert torch.allclose(image.std(), torch.tensor(18.4987), atol=1e-3)
...
def test_png_train_dataset(
    ...
    assert target == 0
    assert image.shape == (1, 64, 48)
    assert image.dtype == torch.float
    assert torch.allclose(image.mean(), torch.tensor(121.1084), atol=1e-3)
    assert torch.allclose(image.std(), torch.tensor(15.7904), atol=1e-3)
